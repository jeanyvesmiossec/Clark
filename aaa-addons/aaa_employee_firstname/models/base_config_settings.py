...
class ResConfigSettings(models.TransientModel):
    ...
    def _employees_for_recalculating(self):
        return self.env['hr.employee'].search([('firstname', '!=', False), ('lastname', '!=', False)])
    ...
    def action_recalculate_partners_name(self):
        ...
        _logger.info("Recalculating names for %d partners.", nb_partners)
        partners._compute_name()
        _logger.info("%d partners updated.", nb_partners)
        employees = self._employees_for_recalculating()
        nb_employees = len(employees)
        _logger.info("Recalculating names for %d employees.", nb_employees)
        employees._compute_name()
        _logger.info("%d employees updated.", nb_employees)
        return True
