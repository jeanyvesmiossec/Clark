...
class ResPartner(models.Model):
    ...
    firstname = fields.Char(sting="First name", index=True)
    ...
    def write(self, vals):
        ...
        if 'is_modify_name' in vals:
            ...
            for partner in self:
                ...
                for user in user_obj.sudo().search([('partner_id', '=', partner_id)], limit=1):
                    employees = user.sudo().employee_ids
                    employees.sudo().write({'lastname': lastname, 'firstname': firstname})
#                 for employee in employee_obj.sudo().search([('address_home_id', '=', partner_id)]):
#                     user = employee.sudo().user_id
#                     if user:
#                         user.sudo().write({'lastname': lastname, 'firstname': firstname})
#                         user.sudo().employee_ids.write({'lastname': lastname, 'firstname': firstname})
#                     else:
#                         employee.sudo().write({'lastname': lastname, 'firstname': firstname})
        return res
