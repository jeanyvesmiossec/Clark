...
class AzureADCalendar(models.Model):
    ...
    def sync(self):
        ...
        for ad_event in changes:
            ...
            link = azure_ad_record_link_obj.with_context(active_test=False).search([('data_id', '=', ad_event.uid)])

            if link and not link.active:
                link.active = True
                link.user_id = ad_event.user.id

            if link and link.record:
                # Already in Odoo, patch
                if ad_event.is_deleted:
                    if not link.record.outlook_owner_email or (link.record.from_outlook and self.azure_ad_user_id.email and link.record.outlook_owner_email.lower() == self.azure_ad_user_id.email.lower()):
            ...
            # New event for current user
                ...
                if calendar_event_id:
                    ...
                    create_user = SUPERUSER_ID
                    outlook_owner_user_ids = self.env['azure.ad.user'].search([('email', '=ilike', ad_event.owner_email)]).mapped('partner_id.user_ids')

                    if outlook_owner_user_ids and self.env['calendar.event'].sudo(outlook_owner_user_ids[0]).check_access_rights('create', raise_exception=False):
                        create_user = outlook_owner_user_ids[0].id

                    # Create odoo calendar event based on parameters provided by outlook calendar event
                    event_id = self.env['calendar.event'].sudo(create_user).with_context({'create_from_outlook': True}).create({
    ...
    def create_outlook_event(self, odoo_event, ad_event, link_attendees=True):
        ...
        for rec in self:
            self.env['azure.ad.user.record.link'].sudo().create({
                'user_id': rec.azure_ad_user_id.id,
                'data_domain': EVENTS_DATA_DOMAIN,
                'create_domain': EVENTS_CREATE_DOMAIN % rec.uid,
    ...
    def post(self):
        for rec in self:
            rec.uid = rec.azure_ad_user_id.post_data(CALENDARS_CREATE_DOMAIN, rec.to_azure_ad_template(), force=True)['Id']
    ...
    def unlink(self):
        for calendar in self:
            ...
            for link in links:
                if link.record.from_outlook and link.record.outlook_owner_email and link.user_id.email and link.record.outlook_owner_email.lower() == link.user_id.email.lower() and link.record:
                    link.record.unlink()
                else:
                    link.unlink()
