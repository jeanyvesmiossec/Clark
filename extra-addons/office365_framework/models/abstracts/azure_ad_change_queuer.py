...
class AzureADChangeQueuer(models.AbstractModel):
        ...
        for name, value in data.items():
            ...
            if name in self:
                if name in original_values:
                    ...
                    if hasattr(self[name], 'ids') and isinstance(value, list) and len(value) == 1 and len(value[0]) == 3:
                        # Relation type
                        ...
                        if self[name]['id'] == value:
                            continue
                    elif type(self[name]) == datetime:
                        # Datetime type
                        if fields.Datetime.to_string(self[name]) == value:
                            continue
                    elif type(self[name]) == date:
                        # Datetime type
                        if fields.Date.to_string(self[name]) == value:
                            continue
                    else:
                        # Simple type
                        if self[name] == value:
    ...
    def create_changed_item(self, vals, time, record):
        changed_values = self._get_observed_changed_values_simple_types(vals)

        if not changed_values:
            return

        self.sudo().env['azure.ad.change.queue.item'].create({
            'change': json.dumps(changed_values),
            'time': time,
    ...
    def _get_observed_changed_values_simple_types(self, values):
        ...
        for k, v in values.items():
            if k in observed_keys:
                if type(v) == datetime:
                    changed_values[k] = fields.Datetime.to_string(v)
                elif type(v) == date:
                    changed_values[k] = fields.Date.to_string(v)
                else:
                    changed_values[k] = v
