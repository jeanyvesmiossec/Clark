    ...
    def sync_data(self):
        try:
            if not self.client_id or not self.redirect_url or not self.secret:
                ...
                context['message'] = 'Process has Succeed!'
...
class Office365UserSettings(models.Model):
    ...
    login_url = fields.Char('Login URL', compute='compute_url', readonly=True)
    ...
    def compute_url(self):
        ...
        if settings and settings.client_id and settings.redirect_url:
    ...
    def test_connectiom(self):
        ...
        try:
            ...
            if 'error' in json.loads(response.decode('utf-8')) and json.loads(response.decode('utf-8'))['error']:
                ...
                self.env.cr.commit()
                context = dict(self._context)
                context['message'] = 'Token has been generated!'
                return self.message_wizard(context)
    ...
    def export_calendar(self):
        ...
        if self.env.user.token:
            try:
                ...
                meetings = self.env['calendar.event'].search([("create_uid", '=', self.env.user.id)])
        ...
        if self.env.user.token:
            try:
                if self.env.user.token:
                    ...
                    odoo_contacts = self.env['res.partner'].search([('company_id','=',self.env.user.id),('company_id','=',None)])
                    office_contact = []
                    count = 0
                    ...
                    while True:
                        ...
                        if 'value' in response:
                            for each_contact in response['value']:
                                ...
                                if ('emailAddresses' not in each_contact or not each_contact['displayName']) and  ('emailAddresses' not in each_contact or not each_contact['emailAddresses']) and (not 'businessPhones' in each_contact or not each_contact['businessPhones']):
                                    continue
                                odoo_cust = self.env['res.partner'].search(
                                    [('office_contact_id', '=', each_contact['id'])])
                                if not odoo_cust:
                                    print('3')
                                    if ('emailAddresses' in each_contact and len(each_contact['emailAddresses']) > 0 and ('emailAddresses' in each_contact and each_contact['emailAddresses'][0]['address']!= None)) or (('mobilePhone' in each_contact and each_contact['mobilePhone'] or len('homePhones' in each_contact and each_contact['homePhones'])>0 or len('businessPhones' in each_contact and each_contact['businessPhones'])>0)) :
                                        print('4')
                                        if each_contact['emailAddresses'] and each_contact['emailAddresses'][0]['address']:
                                        ...
                                        if phone and email_address:
                                            if not self.env['res.partner'].search(['|',('email', '=', email_address),('phone','=',phone)]):

                                                contact_data ={
                                                    'company_id' : self.env.user.company_id.id,
                                                    'name': each_contact['displayName'] if 'displayName' in each_contact else
                                                    email_address,
                                                    ...
                                                    'city': each_contact['homeAddress']['city'] if 'city' in each_contact[
                                                        'homeAddress'] and each_contact[
                                                                                                       'homeAddress'] else None,
